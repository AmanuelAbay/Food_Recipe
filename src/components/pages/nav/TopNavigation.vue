<template>
    <div
        class="flex justify-between py-3 md:pl-20 sm:pl-5 pr-10 items-center border-b-100 border-black-900 bg-gray-100">
        <div class="flex justify-start items-center">
            ...
            <div>
                <div v-if="this.$route.path=='/foods' || this.$route.path=='/'"
                    ...
                    <input @input="this.$store.dispatch('setSearch',this.search)" v-model="search" type="text"
                        placeholder="Search Food..." autocomplete="off" class="text-base placeholder-gray ml-1">
                </div>
            </div>
        </div>
        ...
        <div class="flex justify-end items-center pr-5 space-x-5">
            <div v-if="this.$route.path=='/foods' || this.$route.path=='/'" class="">
                <select v-if="loggedIn" v-model="category" @change="this.$store.dispatch('setCategory',category)"
...
export default {
    ...
    methods: {
        visible() {
            if ((this.$route.path === "/signin") || (this.$route.path === "/signup")) { return false; }
            else return true
